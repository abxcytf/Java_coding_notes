...
to parse the code and return whether it is valid. 
...
and unmatched or closed tags with invalid TAG_NAME. Otherwise, the TAG_CONTENT is invalid.
...
The cdata has the following format : <![CDATA[CDATA_CONTENT]]>. The range of CDATA_CONTENT is 
    ...
    public boolean isValid(String code) {
        ...
        //for (int i = 0; i < code.length();) {
            ...
            if (code.startsWith("<![CDATA[", i)) {
                //tag content
                //test if the string start with certain contect at index i
                int j = i + 9;
                i = code.indexOf("]]>", j);
                if (i < 0) {
                    return false;
                ...
                i = code.indexOf('>', j);
                if (i < 0 || i == j || i - j > 9) {
                    //i < 0 means '>' not existing
                    //i == j means no tag name
                    //i - j > 9 means tag name too long
                    return false;
                }
                for (int k = j; k < i; k++) {
                    if (!Character.isUpperCase(code.charAt(k))) {
                        return false;
                    } //if not upper case, not valid
                }
                String s = code.substring(j, i++);
                if (stack.isEmpty() || !stack.pollLast().equals(s)) {
                    //if stack is empty or the tag name is not matched
                    return false;
                }
            } else if (code.startsWith("<", i)) {
                //start tag name
