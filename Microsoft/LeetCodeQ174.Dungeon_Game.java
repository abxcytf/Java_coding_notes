...
initially positioned in the top-left room and must fight his way through the dungeon to rescue the princess.
...
public class Solution {
    ...
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        int[][] hp = new int[m][n];
        //initialization
        hp[m - 1][n - 1] = 1 - dungeon[m - 1][n - 1] > 0 ? 1 - dungeon[m - 1][n - 1] : 1;
        ...
        for (int i = m - 2; i >= 0; i--) {
            for (int j = n - 2; j >= 0; j--) {
                int temp = Math.min(hp[i][j + 1], hp[i + 1][j]) - dungeon[i][j];
                hp[i][j] = temp > 0 ? temp : 1;
            }
        }
